...
def cnn1_double_mnist_model(logits=True, input_range_type=2, pre_filter=lambda x:x):
    input_shape=(28, 28, 1)
    nb_filters = 64
    nb_denses = [400,400,10]##original 200 200 10
    return cnn1_double_model(input_shape, nb_filters, nb_denses, logits, input_range_type, pre_filter)


def cnn1_double_cifar10_model(logits=True, input_range_type=2, pre_filter=lambda x:x):
    input_shape=(32, 32, 3)
    nb_filters = 128
    nb_denses = [512,512,10]
    return cnn1_double_model(input_shape, nb_filters, nb_denses, logits, input_range_type, pre_filter)


def cnn1_double_model(input_shape, nb_filters, nb_denses, logits, input_range_type, pre_filter):
    """
