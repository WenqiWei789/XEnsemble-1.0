...
def cnn2_mnist_model(logits=False, input_range_type=1, pre_filter=lambda x:x):
    input_shape = (28, 28, 1)
    nb_filters = 64
    nb_classes = 10
    return cnn2_model(input_shape, nb_filters, nb_classes, logits, input_range_type, pre_filter)


def cnn2_cifar10_model(logits=False, input_range_type=1, pre_filter=lambda x:x):
    input_shape = (32, 32, 3)
    nb_filters = 64
    nb_classes = 10
    return cnn2_model(input_shape, nb_filters, nb_classes, logits, input_range_type, pre_filter)
...
def cnn2_LFW_model(logits=False, input_range_type=1, pre_filter=lambda x:x):
    input_shape = (64, 64, 3)
    nb_filters = 64
    nb_classes = 106
    return cnn2_model(input_shape, nb_filters, nb_classes, logits, input_range_type, pre_filter)
...
def cnn2_model(input_shape, nb_filters, nb_classes, logits, input_range_type, pre_filter):
    """
    ...
    if input_range_type == 1:
        ...
        # Don't need to do scaling for cnn2 models, as it is the input range by default.
        scaler = lambda x: x
