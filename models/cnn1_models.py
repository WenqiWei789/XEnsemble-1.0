...
def cnn1_mnist_model(logits=True, input_range_type=2, pre_filter=lambda x:x):
    input_shape=(28, 28, 1)
    nb_filters = 32
    nb_denses = [200,200,10]
    return cnn1_model(input_shape, nb_filters, nb_denses, logits, input_range_type, pre_filter)


def cnn1_cifar10_model(logits=True, input_range_type=2, pre_filter=lambda x:x):
    input_shape=(32, 32, 3)
    nb_filters = 64
...
def cnn1_LFW_model(logits=True, input_range_type=2, pre_filter=lambda x:x):
    input_shape=(64, 64, 3)
    nb_filters = 64
    nb_denses = [256,256,106]
    return cnn1_model(input_shape, nb_filters, nb_denses, logits, input_range_type, pre_filter)


def cnn1_model(input_shape, nb_filters, nb_denses, logits, input_range_type, pre_filter):
    """
    ...
    if input_range_type == 1:
        ...
        # Don't need to do scaling for cnn1 models, as it is the input range by default.
        scaler = lambda x: x
