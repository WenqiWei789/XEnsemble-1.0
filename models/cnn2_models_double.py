...
def cnn2_double_mnist_model(logits=False, input_range_type=1, pre_filter=lambda x:x):
    input_shape = (28, 28, 1)
    nb_filters = 128
    nb_classes = 10
    return cnn2_double_model(input_shape, nb_filters, nb_classes, logits, input_range_type, pre_filter)


def cnn2_double_cifar10_model(logits=False, input_range_type=1, pre_filter=lambda x:x):
    input_shape = (32, 32, 3)
    nb_filters = 128
    nb_classes = 10
    return cnn2_double_model(input_shape, nb_filters, nb_classes, logits, input_range_type, pre_filter)
...
def cnn2_double_model(input_shape, nb_filters, nb_classes, logits, input_range_type, pre_filter):
    """
