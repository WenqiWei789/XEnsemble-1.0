...
#         layers = [
#             Conv2D(32, (3, 3), padding='valid', input_shape=(28, 28, 1)),  # 0
#             Activation('relu'),  # 1
#             Conv2D(32, (3, 3)),  # 2
#             Activation('relu'),  # 3
...
def get_lids_random_batch(model, X, X_noisy, X_adv, dataset, k=10, batch_size=10):
    """
    ...
    # def estimate(i_batch):
    #     start = i_batch * batch_size
    #     end = np.minimum(100, (i_batch + 1) * batch_size)
    #     n_feed = end - start
    #     lid_batch = np.zeros(shape=(n_feed, lid_dim))
    #     lid_batch_adv = np.zeros(shape=(n_feed, lid_dim))
    #     lid_batch_noisy = np.zeros(shape=(n_feed, lid_dim))
    #     for i, func in enumerate(funcs):
    #         X_act = func([X_true[start:end], 0])[0]
    #         X_act = np.asarray(X_act, dtype=np.float32).reshape((n_feed, -1))
    #         # print("X_act: ", X_act.shape)
    #
    #         X_adv_act = func([X_adv[start:end], 0])[0]
...
def block_split(X, Y):
    ...
    :return: 
    """
    print("Isolated split 50%, 50% for training and testing")
    num_samples = X.shape[0]
