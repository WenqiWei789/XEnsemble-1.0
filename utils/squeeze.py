...
def median_filter_py(x, width, height=-1):
    ...
    if height == -1:
        height = width
    #x = random_noise(x,mode='s&p',seed=None,clip=True)
    #x = random_noise(x,var=0.12*0.12)

    x = random_noise(x, mode='gaussian', var=0.5 * 0.5) # try 0.1 and 0.5

    #return denoise_wavelet(x,multichannel=False,convert2ycbcr=True,sigma=0.12,mode='soft')
